...
========================
Post-processes the FR 400kV XIIDM network (node-breaker topology) to
...
sys.path.insert(
    0,
    "/home/marotant/dev/AntiGravity/ExpertAssist/venv_expert_assist_py310/lib/python3.10/site-packages",
)
...
log.info(f"  Eligible for double-busbar (>={MIN_BRANCHES} branches): {len(eligible_vls)}")
...
# Load node counter from conversion
...
for vl_id, branches in sorted(eligible_vls.items()):
    ...
    all_new_switches.extend([
        {"id": coupl_d1_id, "voltage_level_id": vl_id,
         "node1": 0, "node2": coupl_nodes[0],
         "kind": "DISCONNECTOR", "name": "Coupler D1",
         "open": False},
        {"id": coupl_bk_id, "voltage_level_id": vl_id,
         "node1": coupl_nodes[0], "node2": coupl_nodes[1],
         "kind": "BREAKER", "name": "Coupler BK",
         "open": False},
        {"id": coupl_d2_id, "voltage_level_id": vl_id,
         "node1": coupl_nodes[1], "node2": 1,
         "kind": "DISCONNECTOR", "name": "Coupler D2",
         "open": False},
    ])
    ...
    for disco_list in [branch_discos, gen_discos, load_discos]:
        for i, (disco_id, intermediate_node) in enumerate(disco_list):
            ...
            all_new_switches.append({
                ...
                "kind": "DISCONNECTOR",
                "name": "SA.2",
                "open": i % 2 == 0,  # SA.2 open for even (BBS1-assigned), closed for odd
            })
            total_new_discos += 1
...
if discos_to_open:
    n.update_switches(pd.DataFrame(
        {"open": [True] * len(discos_to_open)},
        index=discos_to_open
    ))
...
log.info(f"  Round-trip: {len(n2.get_buses())} el. buses, {len(n2.get_busbar_sections())} BBS, "
...
try:
    sys.path.insert(0, "/home/marotant/dev/Expert_op4grid_recommender")
    ...
    for action_id, action_data in sorted(coupler_actions.items()):
        ...
        if -1 in all_buses:
            log.error(f"  FAIL {action_id}: disconnected elements! buses={sorted(all_buses)}")
            n_fail += 1
        elif n_changed == 0:
...
log.info(f"  Island buses: {len(nan_buses)}" + (" PASS" if len(nan_buses) == 0 else " WARN"))
...
log.info("Done!")
log.info("=" * 70)
log.info(f"  Topology: NODE_BREAKER with double-busbar (2 DISCOs per element)")
...
log.info(f"  Actions: {len(actions)} (incl. {sum(1 for k in actions if k.startswith('open_coupler_'))} coupler openings)")
log.info("=" * 70)
